        ...
        <div class="col-sm-6">
            <ol class="breadcrumb" style="float: right;">
                <li class="breadcrumb-item">
                    <a href="{{ route('dosen.proposal.index') }}">Pengajuan Proposal</a>
                </li>
            </ol>
        </div>
    </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="form-verifikasi">
                        ...
                        @method('PUT')
                        <div class="form-group">
                            <label for="status_dosen">Status Pembimbing</label>
                            <select class="form-control select2" id="status_dosen" name="status_dosen" required>
                                <option value=""></option>
                            </select>
                        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $('#table-proposal').DataTable({
                ajax: {
                    url: "{{ route('dosen.proposal.index') }}",
                    type: "GET",
                    data: function (d) {
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row, meta) {
                            return `
                                <button class="btn btn-info btn-sm btn-confirm" type="button" data-id="${row.id}" data-status="${row.status_dosen}" data-bs-toggle="modal" data-bs-target="#verificationModal">
            ...
            $('#table-proposal').on('click', '.btn-confirm', function() {
                ...
                var statusOptions = `
                    <option value="menunggu" ${statusDosen === 'menunggu' ? 'selected' : ''}>Menunggu</option>
                    <option value="diterima" ${statusDosen === 'diterima' ? 'selected' : ''}>Diterima</option>
                    <option value="ditolak" ${statusDosen === 'ditolak' ? 'selected' : ''}>Ditolak</option>
                `;

                $('#status_dosen').html(statusOptions);
            });

            $('#verifikasi-status').click(function() {
                var formData = $('#form-verifikasi').serialize();
                var proposalId = $('#id_proposal').val();
                var url = "{{ route('dosen.proposal.verifikasi', ':id') }}";
