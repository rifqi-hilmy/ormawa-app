        ...
        <div class="col-sm-6">
            <ol class="breadcrumb" style="float: right;">
                <li class="breadcrumb-item">
                    <a href="{{ route('mahasiswa.proposal.index') }}">Pengajuan Proposal</a>
                </li>
            </ol>
        </div>
    </div>
                ...
                <div class="row align-items-end">
                    ...
                    <div class="col-md-6 col-lg-3 mb-3 mb-md-4">
                        <div class="d-flex align-items-end justify-content-end">
                            <a href="{{ route('mahasiswa.proposal.create') }}" class="btn btn-primary"><i
                                    class="bx bx-plus"></i>
                                Tambah Baru</a>
                        </div>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $('#table-proposal').DataTable({
                ajax: {
                    url: "{{ route('mahasiswa.proposal.index') }}",
                    type: "GET",
                    data: function (d) {
            ...
            $('#table-proposal tbody').on('click', '.btn-edit', function(event) {
                ...
                var path = "{{ route('mahasiswa.proposal.edit', ':slug') }}";
            ...
            $('#table-proposal tbody').on('click', '.btn-delete', function(event) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            type: "DELETE",
                            url: "{{ route('mahasiswa.proposal.destroy') }}",
                            data: {
                                id: data.id,
                                _token: "{{ csrf_token() }}"
                            },
