                ...
                <div class="row align-items-end">
                    ...
                    <div class="col-md-4 col-lg-4 mb-3 mb-md-4">
                        <div class="d-flex align-items-end justify-content-end">
                            <a href="{{ route('mahasiswa.agenda.create') }}" class="btn btn-primary"><i
                                    class="bx bx-plus"></i>
                                Tambah Baru</a>
                        </div>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $('#table-agenda').DataTable({
                ajax: {
                    url: "{{ route('mahasiswa.agenda.index') }}",
                    type: "GET",
                },
                ordering: false,
                processing: true,
            ...
            $('#table-agenda tbody').on('click', '.btn-edit', function(event) {
                ...
                var path = "{{ route('mahasiswa.agenda.edit', ':slug') }}";
            ...
            $('#table-agenda tbody').on('click', '.btn-delete', function(event) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            type: "DELETE",
                            url: "{{ route('mahasiswa.agenda.destroy') }}",
                            data: {
                                id: data.id,
                                _token: "{{ csrf_token() }}"
                            },
